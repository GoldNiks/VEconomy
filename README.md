...
- **FTB Quests — награды деньгами.** Два способа:
  - *«Кастомная награда»*: сумма берётся из названия награды — первое число
    (`500`, `1500 монет`). Не требует мода на клиенте.
  - *Автоначисление по главам*: при завершении любого квеста игроки команды
    автоматически получают сумму за квест. Суммы задаются таблицей по главам
    в `config/veconomy-quests.json` (создаётся автоматически, перечитывается
    командой `/economy admin reload`):
    ...
    Название главы должно совпадать с названием в FTB Quests. Так удобно делать
    нарастающие награды: дальше глава — больше сумма. Повторное начисление за
    тот же квест исключено идемпотентным ключом. Тип операции — `QUEST_REWARD`.
