...
- Полностью настраиваемые конфиги в `config/VMods/VEconomy/`, перечитываются в рантайме.
...
- **FTB Quests — награды деньгами.** Рекомендуемый способ — видимая общая награда
  через встроенную *Command Reward*. В редакторе награды укажите:

  - `Title`: сумму общего фонда, например `⛃50` (можно и просто `50`);
  - `Command`: `/economy quest-reward {quest}`;
  - `Team Reward`: включено;
  - `Elevate Permissions`: включено;
  - `Silent`: включено.

  Игроки видят сумму прямо в квесте. После получения награды VEconomy проверяет,
  что квест завершён этой командой, делит указанный фонд поровну между текущими
  участниками, а остаток отправляет в казну. Команда не принимает сумму или UUID,
  поэтому подменить получателя через FTB-команду нельзя. Повторная выдача защищена
  идемпотентными ключами.

  Для квеста с такой Command Reward нельзя одновременно задавать автонаграду в
  `veconomy-quests.json`: при конфликте VEconomy отклонит выдачу и запишет ошибку
  в серверный лог.

  Полная пошаговая настройка и проверка: [денежные награды VEconomy в FTB Quests](docs/FTB_QUESTS_MONEY_REWARDS.md).

  Также сохранены два прежних способа:
  ...
  - *Автоначисление по главам*: при завершении любого квеста команда получает
    **фиксированный фонд** за квест, который делится между участниками команды
    ...
    дополнительные монеты. Суммы задаются в `config/VMods/VEconomy/veconomy-quests.json`
...
## Конфиг `config/VMods/VEconomy/economy-core.toml`

Все файлы VEconomy хранятся только в собственном каталоге
`config/VMods/VEconomy/`, поэтому мод не затрагивает каталоги других модов
серии V. При обновлении прежние конфиги из корня `config/` копируются туда
только при отсутствии одноимённого целевого файла: существующие файлы не
перезаписываются, а исходники не удаляются.
...
>   (`economy-core.toml`, `veconomy-audit.json`, `veconomy-milestones.json`,
...
2. В `config/VMods/VEconomy/economy-core.toml` укажите `type = "mysql"` и заполните `[database.mysql]`.
...
- `config/VMods/VEconomy/` — изолированные конфиги мода + тестируемый снимок `EconomySettings`.
